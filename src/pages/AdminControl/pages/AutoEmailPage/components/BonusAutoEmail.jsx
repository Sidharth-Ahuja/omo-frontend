...
const BonusAutoEmail = () => {
  ...
  const fetchEmailTemplate = async () => {
    try {
      const docRef = await doc(fireStore, 'auto-email-templates', 'bonus')
  ...
  const handleImageUpload = async (event, field) => {
    ...
    if (file) {
      ...
      const storagePath = `auto-email-templates-images/bonus/${field}`
        ...
        <div className='max-w-3xl mx-auto p-8 rounded-lg shadow-lg'>
          ...
          <div className='text-white mt-10'>
            ...
            <p className='text-lg'>
              ...
              {isEditing ? (
                <textarea
                  value={formData.ExclusivePrizesText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      ExclusivePrizesText: e.target.value,
                    })
                  }
            ...
            <p className='text-lg'>
              ...
              {isEditing ? (
                <textarea
                  value={formData.MonthlySpecialText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      MonthlySpecialText: e.target.value,
                    })
                  }
                  className='text-white bg-transparent border p-2 w-full font-light'
                />
            ...
            <p className='text-lg mb-6'>
              ...
              {isEditing ? (
                <textarea
                  value={formData.BonusPointsText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      BonusPointsText: e.target.value,
                    })
                  }
                  className='text-white bg-transparent border p-2 w-full font-light'
                />
              ) : (
                <span className='font-light'>{emailData.BonusPointsText}</span>
              )}
            </p>
            ...
            {isEditing ? (
              <textarea
                value={formData.HowToClaimText}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    HowToClaimText: e.target.value,
                  })
                }
                rows='4'
