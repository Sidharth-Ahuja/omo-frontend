...
const DepositAutoEmail = () => {
  ...
  const fetchEmailTemplate = async () => {
    try {
      const docRef = await doc(fireStore, 'auto-email-templates', 'deposit')
  ...
  const handleImageUpload = async (event, field) => {
    ...
    if (file) {
      ...
      const storagePath = `auto-email-templates-images/deposit/${field}`
      ...
      >
        <div className='max-w-3xl mx-auto p-8 rounded-lg shadow-lg'>
          ...
          <div className='text-white mt-10'>
            ...
            <p className='text-lg'>
              <span className='text-[#FCC100] font-semibold'>Amount Purchased: </span>
              {isEditing ? (
                <textarea
                  value={formData.AmountPurchasedText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      AmountPurchasedText: e.target.value,
                    })
                  }
            ...
            <p className='text-lg'>
              ...
              {isEditing ? (
                <textarea
                  value={formData.TransactionIDText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      TransactionIDText: e.target.value,
                    })
                  }
                  className='text-white bg-transparent border p-2 w-full font-light'
                />
            ...
            <p className='text-lg mb-6'>
              ...
              {isEditing ? (
                <textarea
                  value={formData.DateAndTimeText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      DateAndTimeText: e.target.value,
                    })
                  }
                  className='text-white bg-transparent border p-2 w-full font-light'
                />
              ) : (
                <span className='font-light'>{emailData.DateAndTimeText}</span>
              )}
            </p>
...
export default DepositAutoEmail
