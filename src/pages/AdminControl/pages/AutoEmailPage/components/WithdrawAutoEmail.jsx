...
const WithdrawAutoEmail = () => {
  ...
  const fetchEmailTemplate = async () => {
    try {
      const docRef = await doc(fireStore, 'auto-email-templates', 'withdraw')
  ...
  const handleImageUpload = async (event, field) => {
    ...
    if (file) {
      ...
      const storagePath = `auto-email-templates-images/withdraw/${field}`
      ...
      >
        <div className='max-w-3xl mx-auto p-8 rounded-lg shadow-lg'>
          ...
          <div className='text-white mt-10'>
            ...
            <p className='text-lg'>
              <span className='text-[#FCC100] font-semibold'>Amount: </span>
              {isEditing ? (
                <textarea
                ...
                <span className='font-light'>{emailData.AmountText}</span>
              )}
            </p>
            ...
            <p className='text-lg mb-6'>
              ...
              {isEditing ? (
                <textarea
                  value={formData.InitiatedOnText}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      InitiatedOnText: e.target.value,
                    })
                  }
                  className='text-white bg-transparent border p-2 w-full font-light'
                />
              ) : (
                <span className='font-light'>{emailData.InitiatedOnText}</span>
              )}
            </p>
            <div className='flex justify-center'>
              <button className='bg-[#FD7700] px-6 py-2 text-lg font-semibold'>
                Track Transaction
              </button>
            </div>
          </div>
...
export default WithdrawAutoEmail
